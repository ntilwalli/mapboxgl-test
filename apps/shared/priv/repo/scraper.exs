...
defmodule Main do
  ...
  venue_lng_lat = Map.new(Enum.zip(venue_names, lngLat))
  # IO.inspect venue_lng_lat



  dayExtractor = ~r/(?<week_day>.*day) (?<month>\d{1,2})\/(?<day>\d{1,2})\/(?<year>\d{2})/
  days = Floki.find(html, "font")
  ...
  #IO.inspect dayTables
  listings = Enum.map(dayTables, fn({
    %{"week_day" => week_day, "day" => day, "month" => month, "year" => year}, {_, _, [_ | bodies_tail]}}) -> 
      ...
      Enum.map(bodies_tail, fn({_, _, [{_, _, [
        ...
        lngLat = Map.get(venue_lng_lat, venue_name)
        lng = elem(lngLat, 0)
        lat = elem(lngLat, 1)
