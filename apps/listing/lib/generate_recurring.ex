defmodule Listing.GenerateRecurring do
  ...
  alias Shared.Model.Recurring, as: CuandoRecurring
  alias Shared.Model.Once, as: CuandoOnce
  alias Shared.Model.Listing.Donde.Badslava, as: WhereBadslava

  def start_link(name, registry_name, opts \\ []) do
    GenServer.start_link(__MODULE__, {:ok, registry_name}, name: name)
  end

  def generate(server, user, listing) do
    GenServer.cast(server, {:generate, user, listing})
  end

  def init({:ok, registry_name}) do
    IO.inspect "Starting recurrence worker..."
    #work(registry_name)
    schedule_work(1)
    {:ok, registry_name}
  end

  def handle_cast({:generate, user, listing}, state) do
    generate_recurrences(user, listing, state)
    {:noreply, state}
  end

  def handle_info(:work, registry_name = state) do
    automated_generate(registry_name)
  ...
  defp generate_recurrences(user, listing, registry_name) do
    if listing.type === "recurring" && listing.release === "posted" do
      ...
      |> to_triplet()
      |> to_utc()
      |> process(user, registry_name)
    end
  end

  defp to_triplet(listing) do
  ...
  defp to_utc({recurrable, tz, listing}) do
    ...
    recurrences_utc = CuandoRecurring.between(recurrable, start_of_today, end_datetime, tz) |> Enum.map(fn x -> 
  ...
  defp process({recurrences_utc, listing, tz}, user, registry_name) do 
    start_of_today = get_start_of_today(tz)
    ...
    results = to_be_added |> Enum.map(fn x -> 
        ...
        {:ok, listing} = out = Listing.Worker.add_child_from_struct(pid, single, user)
        out
      end)
  end
