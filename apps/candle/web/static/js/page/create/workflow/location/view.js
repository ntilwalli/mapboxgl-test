...
import {attrs, renderModal} from '../../../utils'
import {getVicinityFromListing, getVicinityString} from './utils'

function renderVicinity(info) {
  const {userLocation, listing} = info.state
  const vicinity = getVicinityFromListing(listing, userLocation)
    ...
    return div(`.input-section`, [
      ...
      , div(`.controller-section`, [
        ...
        div(`.button-section`, [
          ...
          button(`.appNextButton.next-button${state.isValid ? '' : '.disabled'}`, [
            span(`.next-text`, [`Next`]),
