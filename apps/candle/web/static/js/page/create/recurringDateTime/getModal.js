...
import StartDate from './startDate/main'
//import Blah from '../location/searchArea/main'
...
export default function getModal(sources, inputs, {modal, listing}) {
    ...
    if (modal === `startDate`) {
      return Modal(sources, spread(
        inputs, {
        component: (sources, inputs) => StartDate(sources, spread(inputs, {
          listing$
        })),
        props$: O.of({
          headerText: `Change start date`,
          type: `standard`,
