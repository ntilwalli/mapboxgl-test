...
import {findEventTypeOpenMic, findEventTypeShow, findEventTypeTrivia, findEventTypeDance} from '../../../../helpers/listing/utils'
...
export default function main(sources, inputs) {
  ...
  const comedy_component$ = inputs.event_types$
    .map(arr => {
      if (findEventTypeOpenMic(arr) || findEventTypeShow(arr)) {
        return isolate(CheckboxParent)(sources, {
          ...inputs, 
          initial_state$: init_categories$, 
          props$: comedy_props$
        })
  ...
  const comedy_component: any = componentify(comedy_component$, 'output$')
  ...
  const music_component$ = inputs.event_types$
    .map(arr => {
      if (findEventTypeOpenMic(arr) || findEventTypeShow(arr)) {
        return isolate(CheckboxParent)(sources, {
          ...inputs, 
  ...
  const music_component: any = componentify(music_component$, 'output$')
  ...
  const spoken_word_component$ = inputs.event_types$
    .map(arr => {
      if (findEventTypeOpenMic(arr) || findEventTypeShow(arr)) {
        return isolate(CheckboxParent)(sources, {
          ...inputs, 
  ...
  const spoken_word_component: any = componentify(spoken_word_component$, 'output$')
  ...
  const storytelling_component$ = inputs.event_types$
    .map(arr => {
      if (findEventTypeOpenMic(arr) || findEventTypeShow(arr)) {
        return isolate(CheckboxParent)(sources, {
          ...inputs, 
  ...
  const storytelling_component: any = componentify(storytelling_component$, 'output$')
  ...
  const dance_component$ = inputs.event_types$
    .map(arr => {
      if (findEventTypeDance(arr)) {
        return isolate(CheckboxParent)(sources, {
          ...inputs, 
          initial_state$: init_categories$, 
          props$: dance_props$
  ...
  const dance_component: any = componentify(dance_component$, 'output$')
  ...
  const variety_component: any = isolate(CheckboxParent)(sources, {
    ...inputs, 
    initial_state$: init_categories$, 
    props$: variety_props$
  })
