...
function intent(sources) {
  ...
  return {
    ...
    go_to_basics$,
    save_exit$,
    show_errors$: O.merge(go_to_preview$, go_to_basics$).mapTo(true).startWith(false)
  }
}
...
export function main(sources, inputs) {
  ...
  return {
    ...merged,
    DOM: vtree$,
    MessageBus: save_exit_query.error$.map(toMessageBusMainError),
    Router: O.merge(
      merged.Router,
      ...
      actions.go_to_basics$.withLatestFrom(content.output$, (_, state) => {
          return state
        })
        .filter(state => state.valid)
        .map(state => {
