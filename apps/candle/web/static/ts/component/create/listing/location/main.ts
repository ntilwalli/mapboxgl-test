...
import {combineObj, mergeSinks, createProxy} from '../../../../utils'
import {getSearchAreaFromGeolocation} from './utils'


function createVenueAutocomplete(sources, inputs, prop) {
  ...
  const autocompleteInput = AutocompleteInput(sources, {
    results$,
    itemConfigs,
    displayFunction: x => suggestionComponent.name,
    placeholder: `Type venue name here...`,
    styleClass: `.autocomplete-input`
  })

  const suggestionComponent: any = FoursquareSuggestVenues(sources, {
    props$: O.of({}),
  ...
  return {
    ...mergeSinks(autocompleteInput, suggestionComponent), 
    DOM: autocompleteInput.DOM,
    output$: autocompleteInput.selected$,
