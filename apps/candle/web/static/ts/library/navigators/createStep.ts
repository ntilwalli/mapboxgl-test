...
function view(state$, current_step$) {
  return combineObj({state$, current_step$}).map((info: any) => {
    const {notifications, messages} = info.state
    return div('.navbar.navbar-light.bg-faded.container-fluid.fixed-top.navigator', [
      div('.user-navigator.d-flex.fx-j-sb', [
        div('.d-flex.align-items-center', [
          button('.appBrandButton.h-2.hopscotch-icon.btn.btn-link.nav-brand.mr-3', []),
          info.current_step.substring(0, 1).toUpperCase() + info.current_step.substring(1)
        ]),
...
export default function main(sources, inputs) {
  ...
  const vtree$ = view(state$, inputs.current_step$)
