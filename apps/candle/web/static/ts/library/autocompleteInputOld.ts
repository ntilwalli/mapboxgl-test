...
function intent(DOM) {
  ...
  return {
    input$: O.merge(input$, inputFocus$.delay(10)).map((ev: any) => ev.target.value)
    ...
    wantsSuggestions$:
      O.merge(clearField$.mapTo(false), notClearField$.mapTo(true), inputFocus$.mapTo(true), inputBlur$.mapTo(false), inputClick$.mapTo(true)),
    quitAutocomplete$:
...
function renderAutocompleteMenu({suggestions, highlighted}, itemConfigs) {
  ...
  return ul(`.results-container`, suggestions.map((suggestion, index) => {
...
function renderComboBox({suggestions, highlighted, selected}, itemConfigs, displayFunction, placeholder, styleClass, initialText) {
  return div(`.autocomplete-container`, [
    input(`.appAutocompleteable${styleClass}`, {
      attrs: {type: `text`, placeholder, autofocus: true},
      hook: {
...
function view(state$, itemConfigs, displayFunction, placeholder, styleClass, initialText$) {
    ...
    return (
      //div([
        // section(`.section-style`, [
          ...
          renderComboBox({suggestions, highlighted, selected}, itemConfigs, displayFunction, placeholder, styleClass, initialText)
       // ])
      //])
    )
  })
...
function main(sources, {suggester, itemConfigs, displayFunction, placeholder, styleClass, initialText$}) {
  const actions = intent(sources.DOM)

  const suggestionComponent = suggester(sources, {input$: actions.input$.do(x => console.log(`to suggester`, x))})
  const state$ = model(actions, {suggestions$: suggestionComponent.results$}, itemConfigs)
  
  const vtree$ = view(state$, itemConfigs, displayFunction, placeholder, styleClass, initialText$ || O.of(``))
