...
function view(state$, itemConfigs, displayFunction, placeholder, styleClass, initialText$) {
  return combineObj({state$, initialText$}).map(({state, initialText}: any) => {
    ...
    return renderComboBox({suggestions, highlighted, selected}, itemConfigs, displayFunction, placeholder, styleClass, initialText)
  })
}
...
function main(sources, {results$, itemConfigs, displayFunction, placeholder, styleClass, initialText$}) {
