...
function view(state$, props$) {
  return combineObj({state$, props$}).map((info: any) => {
    ...
    return div(`.smart-text-input`, [
      input(`.appInput.text-input${styleClass}`, {class: {disabled}, attrs: {placeholder, name, type, autofocus, value: state.value, disabled}}),
      hasErrors && state.value ? div(`.errors`, {style: {color: "red"}}, state.errors.map(x => div([x]))) : null
    ])
  })
}
...
function main(sources, inputs) {
  ...
  const output$ = state$
      .map((state: any) => state.isValid ? state.validatedValue : undefined)
