defmodule Candle.Email do
  ...
  def forgotten_password(user, token) do
    new_email()
    |> to(user.email)
    |> from("reset@stumplog.com")
    |> subject("Reset password for Stumplog")
    |> text_body("To reset your Stumplog password click the below link.\n\nhttps://www.stumplog.com/i/forgotten?token=#{token}\n\nIf you did not request to reset your password please contact support@stumplog.com")
