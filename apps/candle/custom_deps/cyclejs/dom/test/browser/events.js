...
describe('DOMSource.events()', function () {
  ...
  it('should allow restarting of event streams from isolated components', function (done) {
    ...
    sources.DOM.select(':root').elements().skip(1).take(1).subscribe(root => {
      ...
      setTimeout(() => mouseevent(clickable, 'mousedown'), 5);
      setTimeout(() => mouseevent(clickable, 'mouseup'), 10);
      setTimeout(() => switchSubject.next(undefined), 15);
      setTimeout(() => mouseevent(clickable, 'mousedown'), 20);
      setTimeout(() => mouseevent(clickable, 'mouseup'), 25);
    });
